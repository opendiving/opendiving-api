"""Unit tests for the `/refresh` endpoint (the only thing left in `api.v1.login` - see
`tests/test_auth.py` for the unified email/Google/onboarding flow).
"""
...
class TestRefreshAccessToken:
    ...
    async def test_valid_refresh_token_returns_new_access_token(self, mock_db):
        ...
        with (
            patch("src.app.api.v1.login.verify_token", new_callable=AsyncMock) as mock_verify,
            patch("src.app.api.v1.login.create_access_token", new_callable=AsyncMock) as mock_create,
        ):
