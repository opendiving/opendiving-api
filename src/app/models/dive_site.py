...
class DiveSite(Base, PublicUUIDMixin, TimestampMixin):
    ...
    def __table_args__(cls) -> tuple:
        return (
            # Case-insensitive uniqueness per user on (name, location). COALESCE maps NULL
            # location to '' so two NULL-location sites with the same name are also
            # considered duplicates. Reusing a deleted site's (name, location) needs no
