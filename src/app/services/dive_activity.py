"""How many dives a diver logged in each calendar month.
...
Same split as `dive_gas.py`: `bucket_by_month` is pure - no session, no ORM, no clock -
so the bucketing rules are covered exhaustively without a database
...
def bucket_by_month(dives: Sequence[tuple[datetime, int]]) -> list[DiveActivityPoint]:
    """Count `(start_time, utc_offset_minutes)` pairs into calendar months, oldest first.

    **The month is the dive's own local one**, reconstructed with `combine_start_time`
    ...
    instant would file it under April - and, worse, under a *different* month for a diver
    ...
    Months with no diving are absent rather than zero-filled: the caller plots a fixed
    grid (twelve months, or every year between the first and the last) and has to fill
    the gaps in it regardless, so sending empty buckets would be padding one shape into a
    different one.

    Sorted on the buckets themselves rather than inherited from the input's order, which
    is chronological by *instant* and so can hand back May before April: a dive logged at
    00:30 on the 1st of May in Bangkok is an earlier instant than one at 20:00 on the 30th
    of April in London, and they belong to different months.
    """
    counts = Counter((local.year, local.month) for local in (combine_start_time(*dive) for dive in dives))

    return [DiveActivityPoint(year=year, month=month, dives=count) for (year, month), count in sorted(counts.items())]


async def dive_activity(db: AsyncSession, user_id: int) -> list[DiveActivityPoint]:
    """Every month of a user's diving that contains at least one dive, oldest first.
    ...
    return bucket_by_month([(start_time, offset_minutes) for start_time, offset_minutes in result])
