...
The single entry point into the app: a caller proves ownership of an email address
(magic link) or authenticates with Google, and *only then* do we ask "does an account
already exist for this identity?" (`resolve_identity`, in `services.auth_service`).
