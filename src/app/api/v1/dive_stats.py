...
async def read_dive_stats(
    ...
    if stats is None:
        ...
        return UserDiveStatsRead(user_uuid=user_uuid, created_at=datetime.now(UTC))  # type: ignore[call-arg]

    stats = cast(UserDiveStatsReadInternal, stats)
    return UserDiveStatsRead(**{k: v for k, v in stats.model_dump().items() if k != "user_id"}, user_uuid=user_uuid)
