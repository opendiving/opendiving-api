...
`read_dive_stats` was moved into `users.py` and now shares that module's `"user"`
...
Later, `users.py`'s tag was renamed from `"users"` to `"user"` for the same reason:
a tag names the URL segment its routes hang off, and since "Current-user routes moved
off `/user/me` and `/user/{uuid}` onto a bare `/user`" (below) every route in that
module lives under `/user` - `GET/PATCH/DELETE /user`, `/user/email-change/...`,
`/user/dive-stats`, `/user/gas-use-history`, `/user/dive-activity`. There is no
`/users` collection endpoint left, and the note at the top of `api/v1/users.py`
explains why a public listing isn't coming back soon, so the plural heading in `/docs`
grouped those routes under a path nothing serves. The module file keeps its `users.py`
name - that follows the plural-module convention shared with `dives.py`/`trips.py`,
