...
software: every instance is somebody's own server". Both sentences make self-hosting the product's
identity and then make a universal claim off the back of it, and the second half is the part that
does not hold: **the reader of this README is not necessarily the operator of the instance they
use.** A diver whose club or buddy runs the instance has no shell on that box and no `psql` — the
Postgres database is somebody's, just not theirs. The same division is already recorded under *"The
operator docs carry the consent duty"* and under *"Deleting an account is two changes with a
fortnight between them"*, where the operator is the controller and the diver is the data subject;
the front-page copy was the one place still assuming those were the same person.
...
*capability* rather than leaving it: the lead adjective is "self-hostable", the sentence about
Postgres is conditioned on running your own instance, and the AGPL paragraph's "Run it, change it,
self-host it freely" was already in that register and needed no change at all.
...
the security decisions the operator docs put on them — is unaffected and stays as it is: those
sentences describe a real audience rather than defining the product.
