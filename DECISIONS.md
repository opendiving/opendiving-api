...
the same reasoning that left `certification` without a unique index. `TestADeletedNameFreesItsSlot`
in `tests/test_hard_delete.py` therefore does not grow a course case: there is no slot to free.
