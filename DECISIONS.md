...
rewrapped by hand or left one short line behind. `uv run mdformat *.md docs .github` does it now,
and `.github/workflows/linting.yml` checks it alongside `ruff format --check`.
...
- **Paths are always spelled out** (`*.md docs .github`). Version 1.0 has no `--exclude`, and
